package it.unisa.diem.progettoinf.gruppo25.view;


/**
 * @class ElencoView
 * @brief Gestisce la visualizzazione e l'interazione con l'elenco dei contatti.
 *
 * La classe consente di visualizzare l'elenco completo dei contatti e gestire selezioni
 * o operazioni specifiche su di essi.
 * @author Luisa Crivo, Francesca De Pascale, Antonio Manuel Fedullo
 */
 
public class ElencoView{

}
